...
Spotfire.initialize(async (mod) => {
    ...
    const validateAxes = function() {
        ...
        valid = valid && DataUtility.validateAxisHasExpression(axes, "MD", displayError);
        valid = valid && DataUtility.validateAxisHasExpression(axes, "TVD", displayError);
        valid = valid && DataUtility.validateAxisHasExpression(axes, "Diameter", displayError);
    ...
    const rowToObject = function(row) {
        ...
        object.value = DataUtility.axisHasExpression(axes, "Value") ? row.continuous("Value").value() : null;
        object.perfStartMD = DataUtility.axisHasExpression(axes, "Perf Start MD") ? row.continuous("Perf Start MD").value() : null;
        object.perfEndMD = DataUtility.axisHasExpression(axes, "Perf End MD") ? row.continuous("Perf End MD").value() : null;
    ...
    const drawViz = async function(trellisItemMap) {  
        ...
        const diagramConfig = {
            ...
            marking: marking,
            dark: dark,
            gunWidth: configuration.wellbore.gunWidth,
            gunColor: configuration.wellbore.gunColor,
            perforationBaseWidth: configuration.wellbore.perforationBaseWidth,
            perforationColor: configuration.wellbore.perforationColor,
            perforationLength: configuration.wellbore.perforationLength,
            perforationLeft: configuration.wellbore.perforationLeft,
            perforationRight: configuration.wellbore.perforationRight,
            plugColor: configuration.wellbore.plugColor,
            plugWidth: configuration.wellbore.plugWidth,
            scales: configuration.wellbore.scales
        };
        ...
        trellisItemMap.iterateTrellisItems(function(thisTrellisItem, thisTrellisItemIndex) {
            ...
            if(diagram == null) {
				diagram = new WellboreDiagram(canvasElem, actions, zoomHandler.getZoomRange());
    ...
    const readInitialConfig = async function() {
        ...
        zoomHandler.addEventListener(ZoomHandler.ZOOM_CHANGE_EVENT, function(e) {
            ...
            trellisCollection.iteratePanels(function(thisPanel, thisPanelIdx) {
                const diagram = thisPanel.getDiagram();
                diagram.setZoomRange(zoomHandler.getZoomRange());
                diagram.draw();
            });    
        });
    };
    ...
    const reader = mod.createReader(
        ...
        mod.visualization.axis("Perf Start MD"),
        mod.visualization.axis("Perf End MD"),
    ...
    async function render(colorView, trellisByView, groupByView, layerTypeView, mdView, tvdView, diameterView, valueView, perfStartMDView, perfEndMDView,
            dataView, windowSize) {
        ...
        // Copy the axes over
        axes = {};
        const axesArr = [colorView, trellisByView, groupByView, layerTypeView, mdView, tvdView, diameterView, valueView, perfStartMDView, perfEndMDView];
